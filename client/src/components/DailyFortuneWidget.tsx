...
export default function DailyFortuneWidget({ onViewDetails }: DailyFortuneWidgetProps) {
  ...
  const generateFortune = (sajuRecord: any): string => {
    ...
    // 사주 정보 기반 다양한 운세 생성
    const fortuneMessages = [
      `오늘은 ${sajuRecord.result?.personality || '당신의 운세'}에 따라 좋은 기운이 흐르는 날입니다. 새로운 시도를 해보세요.`,
      `${sajuRecord.result?.luck || '긍정적인 에너지'}를 활용하여 중요한 결정을 내리기에 좋은 타이밍입니다.`,
      `이 ${dayNames[dayOfWeek]}요일은 주변 사람들과의 관계가 더욱 돈독해질 수 있는 날입니다.`,
      `창의력과 직관력이 높아지는 시간대입니다. 새로운 아이디어를 시도해보세요.`,
      `평온함과 안정이 찾아오는 날입니다. 마음을 정리하고 중요한 일에 집중해보세요.`,
      `운이 좋아지는 신호가 보입니다. 긍정적인 마음으로 하루를 시작해보세요.`,
      `오늘은 인간관계에 좋은 날입니다. 소중한 사람들과 시간을 보내보세요.`,
      `재운이 좋은 날입니다. 새로운 기회에 주목해보세요.`,
    ];
