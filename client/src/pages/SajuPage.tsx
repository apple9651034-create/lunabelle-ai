...
import { usePreventDuplicateSubmit } from '@/hooks/usePreventDuplicateSubmit';
...
export default function SajuPage() {
  ...
  const handleAnalyzeSaju = () => {
    ...
    handleAnalyzeWithDuplicateCheck(async () => {
      setIsLoading(true);
      ...
      try {
        ...
        if (!unknownTime) {
          ...
          if (hourIndex === 0) hourValue = 0;
          else hourValue = hourIndex * 2;
        } else {
          hourValue = 12;
        }
  ...
  return (
    <div className="min-h-screen" style={{ background: 'oklch(0.12 0.03 270)' }}>
      ...
      <div className="p-4 space-y-4">
        ...
        {/* Result */}
        {result && (
          <div ref={resultRef} className="p-5 space-y-4" style={cardStyle}>
            <h2 className="text-lg font-bold" style={{ color: 'oklch(0.78 0.15 85)', fontFamily: "'Noto Serif KR', serif" }}>
              분석 결과
            </h2>

            <div className="space-y-2 text-sm" style={{ color: 'oklch(0.85 0.015 90)' }}>
              <p>
                <strong>양력:</strong> {result.solarDate}
              </p>
              <p>
                <strong>음력:</strong> {result.lunarDate}
              </p>
              <p>
                <strong>명식:</strong> {result.fourPillars.yearString} {result.fourPillars.monthString} {result.fourPillars.dayString}{' '}
                {result.fourPillars.hourString}
              </p>
            </div>

            <div className="p-3 rounded-lg" style={{ background: 'oklch(0.20 0.05 270)' }}>
              <p className="text-xs font-semibold mb-2" style={{ color: 'oklch(0.78 0.15 85)' }}>
                성격
              </p>
              <p className="text-sm" style={{ color: 'oklch(0.85 0.015 90)' }}>
                {result.personality}
              </p>
            </div>
